...
# תאימות לפרמטרי כתובת (גרסאות שונות של Streamlit)
try:
...
def normalize_text(s: str) -> str:
    """נירמול יוניקוד, הסרת סימוני כיווניות/תווים בלתי נראים, המרת רווח לא־שביר, וגזירת שוליים."""
...
def only_digits(s: str) -> str:
    """משאיר רק ספרות (תומך גם בספרות לא־לטיניות אחרי NFKC)."""
    s = unicodedata.normalize("NFKC", s or "")
...
def valid_phone(v: str) -> bool:
    ...
    v = re.sub(r"\s+", "", v)
    return bool(re.match(r"^0\d{8,9}$", v))  # 9–10 ספרות כולל קידומת 0
...
if st.session_state.step == 1:
    ...
    if nxt:
        # נירמול לפני בדיקות
        st.session_state.first_name = normalize_text(st.session_state.get("first_name",""))
        st.session_state.last_name  = normalize_text(st.session_state.get("last_name",""))
        st.session_state.nat_id     = only_digits(st.session_state.get("nat_id",""))
        st.session_state.phone      = normalize_text(st.session_state.get("phone",""))
        st.session_state.address    = normalize_text(st.session_state.get("address",""))
        st.session_state.email      = normalize_text(st.session_state.get("email",""))
        if st.session_state.get("mother_tongue") == "אחר...":
            st.session_state.other_mt = normalize_text(st.session_state.get("other_mt",""))
        if "אחר..." in st.session_state.get("extra_langs", []):
            st.session_state.extra_langs_other = normalize_text(st.session_state.get("extra_langs_other",""))
        if st.session_state.get("study_year") == "אחר...":
            st.session_state.study_year_other = normalize_text(st.session_state.get("study_year_other",""))
        if st.session_state.get("mobility") == "אחר...":
            st.session_state.mobility_other = normalize_text(st.session_state.get("mobility_other",""))

        # בדיקות
        errors=[]
        if not st.session_state.first_name: errors.append("יש למלא שם פרטי.")
        if not st.session_state.last_name:  errors.append("יש למלא שם משפחה.")
        if not valid_id(st.session_state.nat_id):   errors.append("ת״ז חייבת להיות 8–9 ספרות.")
        if mother_tongue=="אחר..." and not st.session_state.get("other_mt",""): errors.append("יש לציין שפת אם (אחר).")
        if not extra_langs or ("אחר..." in extra_langs and not st.session_state.get("extra_langs_other","")):
        ...
        if not st.session_state.address:    errors.append("יש למלא כתובת מלאה.")
        if not valid_email(st.session_state.email): errors.append("כתובת דוא״ל אינה תקינה.")
        if study_year=="אחר..." and not st.session_state.get("study_year_other",""):
            errors.append("יש לפרט שנת לימודים (אחר).")
        if not track.strip(): errors.append("יש למלא מסלול לימודים/תואר.")
        if mobility=="אחר..." and not st.session_state.get("mobility_other",""):
...
if st.session_state.step == 6:
    ...
    if send:
        ...
        if not st.session_state.first_name: errors.append("סעיף 1: חסר שם פרטי.")
        if not st.session_state.last_name:  errors.append("סעיף 1: חסר שם משפחה.")
        if not valid_id(st.session_state.nat_id):  errors.append("סעיף 1: ת״ז לא תקינה.")
        if errors:
            st.error("נמצאו שגיאות:"); [st.markdown(f"- :red[{e}]") for e in errors]
        else:
            ...
            row = {
                ...
                "שם_פרטי": st.session_state.first_name,
                "שם_משפחה": st.session_state.last_name,
                "תעודת_זהות": st.session_state.nat_id,
