...
# --- פונקציות עזר לשדות ששומרות ערכים באופן עקבי ---
...
def ti(label, key, placeholder=""):
    """text_input עם טעינת ערך מה־session"""
...
def ta(label, key, height=100, placeholder=""):
    return st.text_area(
        label,
        value=st.session_state.get(key, ""),
        key=key,
...
def sb(label, options, key, placeholder=None):
    """selectbox ששומר ערך (באמצעות index מה־session)"""
    # אם יש ערך קודם – נשתמש בו, אחרת ניקח את הראשון
    val = st.session_state.get(key, options[0])
    return st.selectbox(
        label,
        options,
        index=_idx(options, val),
        key=key,
        placeholder=placeholder,
    )

def ms(label, options, key, max_selections=None, placeholder=None):
    """multiselect ששומר ערכים"""
...
/* ====== עיצוב מודרני + RTL ====== */
:root{ --bg-1:#e0f7fa; --bg-2:#ede7f6; --bg-3:#fff3e0; --bg-4:#fce4ec; --bg-5:#e8f5e9;
