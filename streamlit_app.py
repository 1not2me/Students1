...
RANK_COUNT = 3  # דירוג עד 3 מקומות בלבד
...
with tab2:
    ...
    st.markdown("**בחר/י מוסד לכל מדרגה דירוג (1 = הכי רוצים, 3 = הכי פחות).**")
    ...
    for i in range(1, RANK_COUNT + 1):
        with cols[(i - 1) % 2]:
            ...
            st.session_state[f"rank_{i}"] = st.selectbox(
                ...
                options=opts,
                index=opts.index(st.session_state[f"rank_{i}"]) if st.session_state[f"rank_{i}"] in opts else 0,
...
with tab4:
    ...
    adjustments = st.multiselect(
        ...
        ["הריון","מגבלה רפואית","רגישות למרחב רפואי","אלרגיה חמורה","נכות","רקע משפחתי רגיש","אחר..."],
...
if submitted:
    ...
    if any(s == "— בחר/י —" for s in rank_to_site.values()):
        errors.append("סעיף 2: יש לבחור מוסד בכל מדרגה.")
    if len(set(rank_to_site.values())) != len(rank_to_site.values()):
        errors.append("סעיף 2: קיימת כפילות בבחירת מוסדות.")

    if avg_grade <= 0: errors.append("סעיף 3: יש להזין ממוצע ציונים.")

    if not adjustments_details.strip():
        errors.append("סעיף 4: יש לפרט התאמות (אפשר לכתוב 'אין').")
    ...
    if errors:
        ...
        row = {
            ...
            "שפת_אם": other_mt if mother_tongue == "אחר..." else mother_tongue,
            "שפות_נוספות": "; ".join(extra_langs),
            "טלפון": phone.strip(),
            "כתובת": address.strip(),
            "אימייל": email.strip(),
            ...
            "תחומים_מועדפים": "; ".join(chosen_domains),
            "תחום_מוביל": top_domain,
