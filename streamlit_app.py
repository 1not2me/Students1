...
# אתחול פעם־אחת בלבד
# =========================
if "__bootstrapped" not in st.session_state:
    # ברירות מחדל
    st.session_state.step = st.session_state.get("step", 0)
    if "acks" not in st.session_state:
        st.session_state.acks = {i: False for i in range(5)}
    ...
    # סניטציה ראשונית ושמירה
    sanitize_state()
    save_persisted_state()
    st.session_state.__bootstrapped = True
...
st.multiselect(
    ...
    st.markdown("---")
    # שימי לב: אין value=... (כך לא דורסים ערך קיים)
    st.session_state.acks[0] = st.checkbox("אני מצהיר/ה כי מילאתי פרטים אישיים באופן מדויק. *", key="ack_0")
    save_persisted_state()
    prev_next()
...
if step == 1:
    ...
    st.markdown("---")
    st.session_state.acks[1] = st.checkbox("אני מצהיר/ה כי העדפתי הוזנו במלואן. *", key="ack_1")
    save_persisted_state()
    prev_next()
...
if step == 2:
    ...
    st.markdown("---")
    st.session_state.acks[2] = st.checkbox("אני מצהיר/ה כי הממוצע שהזנתי נכון. *", key="ack_2")
    save_persisted_state()
    prev_next()
...
if step == 3:
    ...
    st.markdown("---")
    st.session_state.acks[3] = st.checkbox("אני מצהיר/ה כי מסרתי מידע מדויק על התאמות. *", key="ack_3")
    save_persisted_state()
    prev_next()
...
if step == 4:
    ...
    st.markdown("---")
    st.session_state.acks[4] = st.checkbox("אני מצהיר/ה כי עניתי בכנות על שאלות המוטיבציה. *", key="ack_4")
    save_persisted_state()
    prev_next()
